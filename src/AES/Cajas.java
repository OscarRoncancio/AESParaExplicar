/*
 * To change this template, choose Tools | Templates
 * and open the template in the editor.
 */

package AES;

/**
 *
 * @author Mercurio
 */
public class Cajas {
    /**
     * Tabla de sustitución inversa S-Box.
     * 
     */
    protected static int[] invSBox = new int[] {  };

  /**
   * Rcon (Round Constant).
   * Es una palabra en la que los 3 bytes màs a la derecha son siempre 0.
   * Ademàs el efecto de un XOR de una palabra con Rcon es para solamente realizar un XOR 
   * sobre los bytes màs a la izquierda de la palabra.
   * 
   * La ronda constante es diferente para cada ronda y esta definida como Rcon[j]=(RC[j],0,0,0)
   * con RC[1]=1,
   * RC[j]=2*RC[j-1] y con multiplicaciòn definida sobre el campo GF(2^8). Los valores de 
   * RC[j] en hexadecimal son:
   *
   */
  protected static int[] rCon = new int[] {  };

/**
 * Tabla de sustituciòn S-Box
 * Matriz de 16x16 que contiene la permutaciòn de todos los 256 posibles valores
 * (8 bits)
 * Cada byte de estado es mapeado en un nuevo byte:
 * Los 4 bits màs a la izquierda del byte son usados como un valor de fila y
 * los 4 bits más a la derecha del byte como valor de columna.
 * Estos valores de fila y columna sirven como indices dentro de la S-Box para
 * seleccionar un ùnico valor de salida de 8 bits.
 * Ejemplo:
 * El valor hexadecimal 95, se refiere a la fila 9, columna 5 de la sBox que
 * contiene el valor 2A.
 *
 */
  protected static int[] sBox = new int[] {  };
}
